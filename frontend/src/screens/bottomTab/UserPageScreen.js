...
export default function UserPageScreen({navigation}) {
  ...
  const renderScene = ({route}) => {
    switch (route.key) {
      case 0:
        return <VoteLike navigation={navigation} myPageData={myPageData} />;
      case 1:
        return <VoteDo navigation={navigation} myPageData={myPageData} />;
      case 2:
        return (
          <VoteWrite
        ...
        <View style={{marginStart: '5%'}}>
          <View>
            ...
            {myPageData.user ? (
              <Text>
                {(() => {
                  if (myPageData.user.genderCode === 'G0101')
                    return <Text>남성</Text>;
                  else return <Text>여성</Text>;
                })()}{' '}
                /
                {(() => {
                  if (myPageData.user.mbti1Code === 'M0101')
                    return <Text> E</Text>;
                  else return <Text> I</Text>;
                })()}
                {(() => {
                  if (myPageData.user.mbti2Code === 'M0201')
                    return <Text>N</Text>;
                  else return <Text>S</Text>;
                })()}
                {(() => {
                  if (myPageData.user.mbti3Code === 'M0301')
                    return <Text>F</Text>;
                  else return <Text>T</Text>;
                })()}
                {(() => {
                  if (myPageData.user.mbti4Code === 'M0401')
                    return <Text>P</Text>;
                  else return <Text>J</Text>;
                })()}{' '}
                /
                {(() => {
                  if (myPageData.user.ageRangeCode === 'A0101')
                    return <Text> 10대 미만</Text>;
                  else if (myPageData.user.ageRangeCode === 'A0102')
                    return <Text> 10대</Text>;
                  else if (myPageData.user.ageRangeCode === 'A0103')
                    return <Text> 20대</Text>;
                  else if (myPageData.user.ageRangeCode === 'A0104')
