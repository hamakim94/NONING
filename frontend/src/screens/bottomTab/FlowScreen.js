  ...
  return (
    <View style={{flex: 1}}>
      ...
      <FlatList
        ...
        renderItem={({item}) => (
          <View style={{height:windowHeight*0.935, flex: 1, borderWidth: 5, backgroundColor: generateColor()}}>
            <View style={{flex: 1, alignItems: 'center', justifyContent: 'center'}}>
                <Text style={{fontWeight: 'bold'}}>#{} #{} #{} 사용자들이</Text>
                <Text style={{fontWeight: 'bold'}}>많이 참여한 논쟁입니다.</Text>
            </View>
            <View style={{flex: 4, borderWidth: 0.5, justifyContent: 'center'}}>
                <View style={{width: '90%', height: '60%', borderWidth: 1, borderRadius: 5, alignSelf: 'center', paddingVertical: '5%'}}>
                    <Text style={{ flex: 3, textAlign: 'center', jusfityContent: 'center', fontSize: 25, fontWeight: 'bold'}}>{item.title}</Text>
                    <View style={{ flex: 1.8, alignSelf: 'center'}} board={boards} user_vote = {user_vote} setUserLike= {setUserVote}>
                ...
                <View style={{flexDirection: 'row', marginLeft: '5%', marginTop: '2.5%'}}> 
                    <Text>작성자 :  </Text>
                    <Image style={{ width:15 ,height:15, borderRadius:50 }} source={{uri : item.writer.img}}></Image>
                    <Text>{item.writer.nickname}</Text>
                </View>
            </View>
            <View board={boards} user_like = {user_like} setUserLike= {setUserLike} 
                  style={{flex: 1.2, alignItems: 'flex-end', jusfityContent: 'center'}}>
                <TouchableOpacity
